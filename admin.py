...
# Re‑use the ``key_store`` instance created in ``auth.py`` to ensure a single source of truth.
...
async def rotate_api_key(key: str, admin: str = Depends(verify_admin_token)):
    """Rotate (regenerate) an existing API key while preserving its settings."""
    ...
    payload = APIKeyCreate(
        name=existing.name,
        puter_token=existing.puter_token,
